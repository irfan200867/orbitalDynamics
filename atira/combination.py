...
def title():
    print('***************************************************************')
    print('* REBOUND simulation of asteroid (w/o Post-Newtonian effects) *')
    print('*                                                             *')
    print('* (c) P2MI 2023 project: MIH & ES                             *')
    print('***************************************************************')
    return
...
for i in range(2):
    ...
    print(f'\n*** SIMULATION OF {out0} WITH {Nout} POINTS, AND {Tint} YR INTEGRATION TIME ***')
    ...
    print(f'*** RESTART FOR ADOPTING SOME CLONING VALUES OF {list(atira.items())[idx][0]} ***')
    ...
    print(f'*** Adding particle #{len(ps)} as clone #{i+1:03d}/{len(combi):03d} of {list(atira.items())[idx][0]} ***')
    print(f"*** Using combination {combi[i]} for its 5-sigma's***")
    print(f'*** Original values of a [au], e, inc [rad], Omega [rad], and omega [rad] '\
          f'as of {datetime.now()} are:')
    print(f'a = {aori:11.9E}, e = {eori:11.9E}, i = {iori:11.9E}, '\
          f'Omega = {Oori:11.9E}, and omega = {oori:11.9E}')
    print(f'*** Cloning values become:')
    print(f'a = {aasi:11.9E}, e = {ecci:11.9E}, i = {inci:11.9E}, '\
          f'Omega = {OMGi:11.9E}, and omega = {omgi:11.9E}')
    print(f'*** CONTINUING SIMULATION {i+1:03d}/{len(combi):03d}, combination {combi[i]}, starting from {datetime.now()}...')
    ...
    print('t,dt,x,y,z,dcSun,dsSun,Rsun,dcMer,dsMer,tRhMer,Rmer,dcVen,dsVen,tRhVen,Rven,dcEar,dsEar,tRhEar,Rear,dcMar,dsMar,tRhMar,Rmar,dcJup,dsJup,tRhJup,Rjup,dcSat,dsSat,tRhSat,Rsat,dcUra,dsUra,tRhUra,Rura,dcNep,dsNep,tRhNep,Rnep,a,perihel,e,Porb,KL,inc,Omg,omg,f,E,J,enc,pln')
    ...
    for l,time in enumerate(times):
        ...
        print(f'{time/2/np.pi:8.3f},{sim.dt/2/np.pi:6.3f},'\
    ...
    print(f'*** Simulation #{i+1:03d}/{len(combi):03d} w/ combination {combi[i]} done. '\
